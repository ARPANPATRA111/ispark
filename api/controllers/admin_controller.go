...
func AdminDownloadCertificate(c *fiber.Ctx) error {
	...
	// Served through the storage layer rather than the local filesystem: in a
	// cloud deployment FilePath is a remote object URL, so os.Stat/c.Download
	// would always report the file as missing.
	ref := cert.FilePath
	if ref == "" {
		ref = cert.FileName
	}

	file, err := storage.Default().Open(ref)
	if err != nil {
	...
	if downloadName == "" {
		downloadName = filepath.Base(ref)
	}

	c.Set(fiber.HeaderContentDisposition, fmt.Sprintf("attachment; filename=%q", downloadName))
	if contentType, ok := allowedCertificateTypes[strings.ToLower(filepath.Ext(downloadName))]; ok {
